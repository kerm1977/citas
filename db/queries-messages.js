...
function saveMessage({ room, senderId, receiverId, type, content, iv, replyTo, groupId }) {
  const id = randomUUID();
  dbRun(
    `INSERT INTO messages (id,room,sender_id,receiver_id,group_id,type,content,iv,reply_to)
     VALUES (?,?,?,?,?,?,?,?,?)`,
    [id, room, senderId, receiverId || null, groupId || null, type || 'text', content, iv || null, replyTo || null]
  );
  return id;
}
...
module.exports = {
  saveMessage, getMessages, getGroupMessages, markRead, deleteMessage, getRoomId, getMessageById, getMessageByContent,
